#> Description: Raycasting for Guardian's Beam.
...
execute as @e[distance=..2.5,tag=!temp.ray_origin,tag=!temp.hit,tag=dvz.oceans_pearl_hitbox] at @s positioned ~-0.55 ~-0.25 ~-0.55 if entity @e[type=minecraft:marker,tag=temp.ray,dx=0.3,dy=0.3,dz=0.3] run tag @s add temp.hit

# Repeat ray casting loop.
execute as @s at @s run function dvz:zombie/ability/skeleton_variant/guardian/beam_loop
