...
execute if entity @s[scores={dvz.guardian.beam.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Beam]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.guardian.beam.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.guardian.beam.cooldown.seconds=1..}] run return 0

# Set the cooldown.
scoreboard players set @s dvz.guardian.beam.cooldown.seconds 3
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:zombie/ability/skeleton_variant/guardian/beam_loop

# Deals damage to all players (and gaurd pearls) who have been hit by the ray. Damage is substantially reduced for zombies.
execute as @a[tag=temp.hit,tag=dvz.dwarf] run damage @s 12 dvz:electric by @p[tag=temp.ray_origin]
execute as @a[tag=temp.hit,tag=dvz.zombie] run damage @s 2 dvz:electric by @p[tag=temp.ray_origin]
execute as @e[tag=temp.hit,tag=dvz.oceans_pearl_hitbox] run damage @s 12 dvz:electric by @p[tag=temp.ray_origin]

# Play a ding sound for the beam shooter if another player was hit.
execute if entity @e[tag=temp.hit,tag=!dvz.zombie.element.electric] run playsound minecraft:entity.arrow.hit_player player @s ~ ~ ~ 1 1 1
