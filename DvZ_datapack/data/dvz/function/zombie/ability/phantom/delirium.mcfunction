...
execute if entity @s[scores={dvz.phantom.delirium.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Delirium]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.phantom.delirium.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.phantom.delirium.cooldown.seconds=1..}] run return 0

# Set a cooldown.
scoreboard players set @s dvz.phantom.delirium.cooldown.seconds 20
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:zombie/ability/phantom/delirium_loop
