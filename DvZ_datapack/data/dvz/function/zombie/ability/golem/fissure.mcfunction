...
execute if entity @s[scores={dvz.golem.fissure.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Fissure]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.golem.fissure.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.golem.fissure.cooldown.seconds=1..}] run return 0
...
# Set the cooldown.
scoreboard players set @s dvz.golem.fissure.cooldown.seconds 5
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:zombie/ability/golem/fissure_loop
