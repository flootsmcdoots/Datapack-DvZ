...
execute unless entity @s[scores={dvz.silverfish.silverfish_egg.count=1..}] run title @s actionbar [ \
  "", \
...
execute unless entity @s[scores={dvz.silverfish.silverfish_egg.count=1..}] run scoreboard players set @s dvz.misc.custom_bar_hide.ticks 20
execute unless entity @s[scores={dvz.silverfish.silverfish_egg.count=1..}] run return 0
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:zombie/ability/silverfish/infest_loop

# Infest surrounding blocks if the ray has hit a dwarf block. 
execute at @n[type=minecraft:marker,tag=temp.ray] if block ~ ~ ~ #dvz:dwarf_blocks run function dvz:zombie/ability/silverfish/infest_infest_blocks
