...
execute if entity @s[scores={dvz.silverfish.roar.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Roar]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.silverfish.roar.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.silverfish.roar.cooldown.seconds=1..}] run return 0
...
# Set the cooldown.
scoreboard players set @s dvz.silverfish.roar.cooldown.seconds 20
...
scoreboard players set @s dvz.silverfish.silverfish_egg.warmup.ticks 400
...
execute as @e[type=minecraft:marker,tag=dvz.infestation,distance=..32] at @s store result score @s temp.infested_block_count run fill ~-1 ~-1 ~-1 ~1 ~1 ~1 minecraft:air replace #dvz:infested_blocks

# Clear all infested blocks in a 32 x 32 x 32 area around the player.
execute store result score @s temp.infested_block_count run fill ~-16 ~-16 ~-16 ~16 ~16 ~16 minecraft:air replace #dvz:infested_blocks

# Add all the blocks cleared to the player's scoreboard.
scoreboard players operation @s temp.infested_block_count += @e[type=minecraft:marker,tag=dvz.infestation,distance=..32] temp.infested_block_count

# Loop that summons a silverfish based on how many infested blocks you cleared.
execute as @s[scores={temp.infested_block_count=1..}] at @s run function dvz:zombie/ability/silverfish/roar_loop

# Loop that summons a silverfish based on how many remaining silverfish eggs you have.
execute as @s[scores={dvz.silverfish.silverfish_egg.count=1..}] at @s run function dvz:zombie/ability/silverfish/roar_loop_2

# Kill all infestation markers in proximity of 32 blocks.
kill @e[type=minecraft:marker,tag=dvz.infestation,distance=..32]
