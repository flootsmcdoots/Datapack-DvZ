...
execute at @r[tag=dvz.dwarf,distance=..64] run summon minecraft:silverfish ~ ~ ~

# Summon an ai silverfish at the player silverfish if there is no dwarf in range.
execute unless entity @a[tag=dvz.dwarf,distance=..64] run summon minecraft:silverfish ~ ~ ~
