...
execute if entity @s[scores={dvz.blaze.fireball.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Fireball]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.blaze.fireball.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.blaze.fireball.cooldown.seconds=1..}] run return 0

# Set cooldown.
scoreboard players set @s dvz.blaze.fireball.cooldown.seconds 4
...
scoreboard players set @n[type=minecraft:small_fireball,tag=temp.processing] dvz.blaze.fireball.lifetime.ticks 50
...
tag @n[type=minecraft:marker,tag=temp.processing] add dvz.projectile.blaze_fireball
...
tag @n[type=minecraft:small_fireball,tag=temp.processing] add dvz.projectile.blaze_fireball
