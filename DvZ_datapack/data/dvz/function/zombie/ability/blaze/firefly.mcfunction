...
execute if entity @s[scores={dvz.blaze.firefly.duration.ticks=1..}] run title @s actionbar [ \
  "", \
  {text:"[Firefly]",bold:true,color:"light_purple"}, \
  {text:" Canceled!",color:"light_purple"} \
]
execute if entity @s[scores={dvz.blaze.firefly.duration.ticks=1..}] run return run function dvz:zombie/ability/blaze/firefly_stop

# Display a fail message and return if the ability is on a cooldown.
execute if entity @s[scores={dvz.blaze.firefly.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Firefly]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.blaze.firefly.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.blaze.firefly.cooldown.seconds=1..}] run return 0

# Check if the player can be teleported forward without clipping into solid blocks.
execute positioned ^ ^ ^1.6 unless block ~-0.3 ~ ~-0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~-0.3 ~ ~0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~0.3 ~ ~-0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~0.3 ~ ~0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~-0.3 ~0.9 ~-0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~-0.3 ~0.9 ~0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~0.3 ~0.9 ~-0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~0.3 ~0.9 ~0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~-0.3 ~1.8 ~-0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~-0.3 ~1.8 ~0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~0.3 ~1.8 ~-0.3 #dvz:go_through run tag @s add temp.fail
execute positioned ^ ^ ^1.6 unless block ~0.3 ~1.8 ~0.3 #dvz:go_through run tag @s add temp.fail
...
# Set the cooldown.
scoreboard players set @s dvz.blaze.firefly.cooldown.seconds 15
...
scoreboard players set @s dvz.blaze.firefly.duration.ticks 40

# Multiply gravity by x0 using an attribute.
attribute @s minecraft:gravity modifier add dvz:blaze.firefly.gravity -1 add_multiplied_total
