...
execute if entity @s[scores={dvz.blaze.heat_wave.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Heat Wave]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.blaze.heat_wave.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.blaze.heat_wave.cooldown.seconds=1..}] run return 0
...
# Set the cooldown.
scoreboard players set @s dvz.blaze.heat_wave.cooldown.seconds 10
