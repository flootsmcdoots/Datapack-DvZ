...
execute if entity @s[scores={dvz.chillager.ice_bridge.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Ice Bridge]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.chillager.ice_bridge.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.chillager.ice_bridge.cooldown.seconds=1..}] run return 0
...
# Set the cooldown.
scoreboard players set @s dvz.chillager.ice_bridge.cooldown.seconds 20
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:zombie/ability/chillager/ice_bridge_loop
...
execute as @e[type=minecraft:marker,tag=temp.ice] at @s run kill @e[type=minecraft:marker,tag=dvz.ice,distance=..0.1]

# Add tags to the ice markers.
tag @e[type=minecraft:marker,tag=temp.ice] add dvz.kill_on_reload
tag @e[type=minecraft:marker,tag=temp.ice] add dvz.ice
...
execute as @s at @n[type=minecraft:marker,tag=temp.ray_origin] run function dvz:zombie/ability/chillager/ice_bridge_loop_2
