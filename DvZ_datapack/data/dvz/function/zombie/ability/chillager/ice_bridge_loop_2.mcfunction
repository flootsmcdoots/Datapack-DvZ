...
scoreboard players operation @n[type=minecraft:marker,tag=temp.ice] dvz.chillager.ice_bridge.ice_melt.ticks = @s temp.ice_melt_base.ticks
...
# Repeat the loop.
execute as @s at @s run function dvz:zombie/ability/chillager/ice_bridge_loop_2
