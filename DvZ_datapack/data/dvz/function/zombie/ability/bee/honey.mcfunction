...
execute if entity @s[scores={dvz.bee.honey.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Honey]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.bee.honey.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.bee.honey.cooldown.seconds=1..}] run return 0

# Set cooldown.
scoreboard players set @s dvz.bee.honey.cooldown.seconds 10
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:zombie/ability/bee/honey_loop

# Apply instant healing and abosrption to nearest targeted zombie.
effect give @p[tag=temp.hit,tag=dvz.zombie] minecraft:instant_health 1 1 true
effect give @p[tag=temp.hit,tag=dvz.zombie] minecraft:instant_health 2 0 true
effect give @p[tag=temp.hit,tag=dvz.zombie] minecraft:absorption 20 1
