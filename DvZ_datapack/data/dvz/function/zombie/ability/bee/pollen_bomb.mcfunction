...
execute if entity @s[scores={dvz.bee.pollen_bomb.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Pollen Bomb]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.bee.pollen_bomb.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.bee.pollen_bomb.cooldown.seconds=1..}] run return 0

# Set the cooldown.
scoreboard players set @s dvz.bee.pollen_bomb.cooldown.seconds 20
...
execute anchored eyes positioned ^ ^ ^ run summon minecraft:splash_potion ~ ~ ~ { \
  ...
  Item:{ \
    ...
    count:1, \
    components:{ \
      "minecraft:item_model":"dvz:pollen_bomb", \
...
summon minecraft:marker ~ ~ ~ {Tags:["dvz.projectile.pollen_bomb","temp.processing"]}
...
scoreboard players set @n[type=minecraft:marker,tag=temp.processing] dvz.marker.pollen_bomb.lifetime.ticks 350
scoreboard players set @n[type=minecraft:marker,tag=temp.processing] dvz.marker.pollen_bomb.heal_cooldown.ticks 0
