...
title @s[predicate=!dvz:is_on_ground] actionbar [ \
  "", \
  {text:"[Hover]",bold:true,color:"red"}, \
  {text:" You need to be on ground to toggle this ability.",color:"red"} \
]
execute as @s[predicate=!dvz:is_on_ground] run return 0
...
# Toggle the ability.
item modify entity @s[tag=temp.use.mainhand,tag=temp.enabled] weapon.mainhand dvz:hover_disable
item modify entity @s[tag=temp.use.mainhand,tag=!temp.enabled] weapon.mainhand dvz:hover_enable
item modify entity @s[tag=temp.use.offhand,tag=temp.enabled] weapon.offhand dvz:hover_disable
item modify entity @s[tag=temp.use.offhand,tag=!temp.enabled] weapon.offhand dvz:hover_enable
...
attribute @s[tag=temp.enabled] minecraft:gravity modifier remove dvz:bee.hover.ascend.gravity
attribute @s[tag=temp.enabled] minecraft:gravity modifier remove dvz:bee.hover.descend.gravity
