...
function dvz:zombie/ability/enderman/create_portal_check_above
...
attribute @s[tag=temp.fail] minecraft:movement_speed modifier remove dvz:enderman.create_portal.movement_speed
attribute @s[tag=temp.fail] minecraft:jump_strength modifier remove dvz:enderman.create_portal.jump_strength
...
tag @s add dvz.zombie.class.enderman.kill
...
execute at @s run summon minecraft:marker ~ ~ ~ {Tags:["dvz.marker.ender_portal","dvz.marker.ender_portal.animation","dvz.kill_on_reload"]}

# Set the cutscene scoreboard to 0.
scoreboard players set @n[type=minecraft:marker,tag=dvz.marker.ender_portal.animation] dvz.marker.ender_portal.animation.ticks 0
