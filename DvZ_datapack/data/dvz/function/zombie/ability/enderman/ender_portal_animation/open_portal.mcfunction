...
# Open the portal
tag @s remove dvz.marker.ender_portal.animation
kill @a[tag=dvz.zombie.class.enderman.kill]
...
execute as @a[tag=dvz.zombie] at @s run function dvz:give/other/ender_eye

# Give every zombie player a suicide pill, because its getting deleted for some reason
execute as @a[tag=dvz.zombie] at @s run function dvz:give/hidden/suicide_pill_slot_0
