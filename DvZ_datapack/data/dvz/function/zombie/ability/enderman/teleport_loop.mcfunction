#> Description: Raycasting loop for the teleport ability.
...
execute unless entity @n[tag=temp.ray_origin,distance=..48] run return 0

# Teleports the ray slightly forward.
tp @s ^ ^ ^0.1

# Repeat ray casting loop.
execute as @s at @s run function dvz:zombie/ability/enderman/teleport_loop
