...
execute if entity @s[scores={dvz.enderman.teleport.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Teleport]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.enderman.teleport.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.enderman.teleport.cooldown.seconds=1..}] run return 0
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:zombie/ability/enderman/teleport_loop

# If the ray has hit a solid block, perform a check to see if you can be teleported there
execute at @n[type=minecraft:marker,tag=temp.ray] unless block ~ ~ ~ #dvz:go_through run function dvz:zombie/ability/enderman/teleport_check
...
execute if entity @e[type=minecraft:marker,tag=temp.teleport] run scoreboard players set @s dvz.enderman.teleport.cooldown.seconds 10
...
execute as @s[tag=dvz.zombie.class.enderman.creating_portal] if entity @e[type=minecraft:marker,tag=temp.teleport] at @s run function dvz:zombie/ability/enderman/create_portal_cancel
