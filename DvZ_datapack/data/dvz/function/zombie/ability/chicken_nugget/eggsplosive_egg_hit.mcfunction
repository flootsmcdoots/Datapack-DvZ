...
execute as @a[distance=..4] at @s run function dvz:zombie/ability/chicken_nugget/eggsplosive_egg_check

# Damage all unobstructed nearby players. Zombies take 90% reduced damage.
execute as @a[tag=temp.hit,tag=dvz.dwarf,distance=..4] run damage @s 20 minecraft:explosion by @p[tag=temp.origin]
execute as @a[tag=temp.hit,tag=dvz.zombie,distance=..4] run damage @s 2.0 minecraft:explosion by @p[tag=temp.origin]

# Damage all obstructed players for 50% damage. Zombies take 90% reduced damage.
execute as @a[tag=!temp.hit,tag=dvz.dwarf,distance=..4] run damage @s 10 minecraft:explosion by @p[tag=temp.origin]
execute as @a[tag=!temp.hit,tag=dvz.zombie,distance=..4] run damage @s 1.0 minecraft:explosion by @p[tag=temp.origin]
