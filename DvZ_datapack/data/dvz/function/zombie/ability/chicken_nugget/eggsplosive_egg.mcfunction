...
execute if entity @s[scores={dvz.chicken_nugget.eggsplosive_egg.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Eggsplosive Egg]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.chicken_nugget.eggsplosive_egg.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.chicken_nugget.eggsplosive_egg.cooldown.seconds=1..}] run return 0

# Set the cooldown.
scoreboard players set @s dvz.chicken_nugget.eggsplosive_egg.cooldown.seconds 3
...
tag @n[type=minecraft:area_effect_cloud,tag=temp.processing] add dvz.projectile.eggsplosive_egg
