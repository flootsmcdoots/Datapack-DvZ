...
execute if entity @s[scores={dvz.spider.web.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Web]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.spider.web.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.spider.web.cooldown.seconds=1..}] run return 0
...
# Set the cooldown.
scoreboard players set @s dvz.spider.web.cooldown.seconds 15
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:zombie/ability/spider/web_loop
...
# can be made to check for a solid block with the following condition: ... at @s "unless block ~ ~ ~ #dvz:go_through" positioned ...
execute as @p[tag=temp.hit] at @s run function dvz:zombie/ability/spider/web_place
execute unless entity @a[tag=temp.hit] as @n[type=minecraft:marker,tag=temp.ray] at @s positioned ^ ^ ^-0.1 run function dvz:zombie/ability/spider/web_place
