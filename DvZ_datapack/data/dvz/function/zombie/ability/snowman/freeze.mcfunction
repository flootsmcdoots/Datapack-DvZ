...
execute if entity @s[scores={dvz.snowman.freeze.cooldown.seconds=1..}] run title @s actionbar [ \
  ...
  {"score":{"name":"@s","objective":"dvz.snowman.freeze.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.snowman.freeze.cooldown.seconds=1..}] run return 0

# Set the cooldown.
scoreboard players set @s dvz.snowman.freeze.cooldown.seconds 30
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:zombie/ability/snowman/freeze_loop
