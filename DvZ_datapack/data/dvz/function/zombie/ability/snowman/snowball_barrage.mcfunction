...
execute if entity @s[scores={dvz.snowman.freeze.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Snowball Barrage]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.snowman.snowball_barrage.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.snowman.snowball_barrage.cooldown.seconds=1..}] run return 0

# Set the cooldown.
scoreboard players set @s dvz.snowman.snowball_barrage.cooldown.seconds 8
...
execute as @s at @s run function dvz:zombie/ability/snowman/snowball_barrage_loop
