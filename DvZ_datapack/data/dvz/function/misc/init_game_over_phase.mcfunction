...
scoreboard players set &dvz dvz.game.phase 6

# Display a different message depending on how the game was ended.
execute unless entity @e[type=minecraft:marker,tag=dvz.marker.shrine_block] run tellraw @a[tag=dvz.joined_game] [ \
  "", \
  {text:"\u1120\u1121\u1122\n",font:"dvz:custom"}, \
...
execute unless entity @e[type=minecraft:player,tag=dvz.dwarf] run tellraw @a[tag=dvz.joined_game] [ \
  "", \
  {text:"\u1120\u1121\u1122\n",font:"dvz:custom"}, \
...
scoreboard players set &dvz temp.const.1200 1200
...
scoreboard players operation &dvz temp.timer.minutes = &dvz dvz.game.timer
scoreboard players operation &dvz temp.timer.minutes /= &dvz temp.const.1200
...
tellraw @a [ \
  ...
  {score:{name:"&dvz",objective:"temp.timer.minutes"}}, \
  {text:" minutes"}, \
]
...
execute at @e[type=minecraft:marker,tag=dvz.marker.shrine_block] run setblock ~ ~ ~ minecraft:gilded_blackstone
