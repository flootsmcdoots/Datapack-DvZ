...
kill @e[type=minecraft:marker,tag=dvz.marker.boss_spawn]

# Force load the chunk.
forceload add ~ ~

# Summon a boss spawn marker.
execute at @s align xyz positioned ~0.5 ~ ~0.5 run summon minecraft:marker ~ ~ ~ {Tags:["dvz.marker.boss_spawn","dvz.kill_on_reload"]}

# Summon a boss spawn display marker.
execute at @s align xyz positioned ~0.25 ~0.25 ~0.25 run summon minecraft:block_display ~ ~ ~ { \
  Tags:["dvz.block_display.boss_spawn","dvz.kill_on_reload"], \
