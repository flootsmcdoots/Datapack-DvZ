#> Description: Set the standard zombie spawn delta location.
...
# Summon a zombie spawn delta marker.
execute at @s align xyz positioned ~0.5 ~ ~0.5 run summon minecraft:marker ~ ~ ~ {Tags:["dvz.marker.zombie_spawn","dvz.marker.zombie_spawn.delta","dvz.kill_on_reload"]}

# Summon a zombie spawn delta seat marker.
execute at @s align xyz positioned ~0.5 ~0.6 ~0.5 run summon minecraft:item_display ~ ~ ~ {Tags:["dvz.item_display.zombie_spawn_seat","dvz.item_display.zombie_spawn_seat.delta","dvz.kill_on_reload"]}

# Summon a zombie spawn delta display marker.
execute at @s align xyz positioned ~0.25 ~0.25 ~0.25 run summon minecraft:block_display ~ ~ ~ { \
  Tags:["dvz.block_display.zombie_spawn","dvz.block_display.zombie_spawn.delta","dvz.kill_on_reload"], \
...
# Summon a zombie spawn delta text marker.
execute at @s align xyz positioned ~0.5 ~1.0 ~0.5 run summon minecraft:text_display ~ ~ ~ { \
  Tags:["dvz.text_display.zombie_spawn","dvz.text_display.zombie_spawn.delta","dvz.kill_on_reload"], \
  text:"Delta", \
  billboard:"center" \
}
...
# Tell all admins which player placed the zombie spawn delta.
tellraw @a[tag=dvz.admin] [ \
  "", \
  {text:"SETUP: ",bold:true,color:"red"}, \
  {"selector":"@s",color:"red"}, \
  {text:" set a Zombie Spawn Location Delta.",color:"red"} \
]
