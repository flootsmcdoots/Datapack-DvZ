#> Description: Remove the zombie spawn beta location.
...
# Tell all admins which player removed the zombie spawn beta location.
tellraw @a[tag=dvz.admin] [ \
  "", \
  {text:"SETUP: ",bold:true,color:"red"}, \
  {"selector":"@s",color:"red"}, \
  {text:" removed the Zombie Spawn Location Beta.",color:"red"} \
]
