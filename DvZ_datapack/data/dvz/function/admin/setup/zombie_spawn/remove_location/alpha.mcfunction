#> Description: Remove the zombie spawn alpha location.
...
# Tell all admins which player removed the zombie spawn alpha location.
tellraw @a[tag=dvz.admin] [ \
  "", \
  {text:"SETUP: ",bold:true,color:"red"}, \
  {"selector":"@s",color:"red"}, \
  {text:" removed the Zombie Spawn Location Alpha.",color:"red"} \
]
