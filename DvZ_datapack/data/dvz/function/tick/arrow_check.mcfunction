...
execute on origin as @s[tag=dvz.dwarf.class.hero.wither_warrior,scores={dvz.wither_warrior.soul_charges.count=1..}] as @n[type=#minecraft:arrows,tag=temp.processing,nbt={weapon:{components:{"minecraft:enchantments":{"dvz:soul_power":1}}}}] at @s run function dvz:dwarf/item/hero/wither_warrior/whispersong_arrow

# Remove the temporary tag from the arrow.
tag @s remove temp.processing

# Tag the arrow as processed.
tag @s add dvz.arrow.processed
