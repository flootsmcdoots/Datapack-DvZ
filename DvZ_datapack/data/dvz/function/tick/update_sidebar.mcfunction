...
execute as @a[tag=dvz.dwarf] run scoreboard players add Dwarves temp.new_player_count 1

# Count zombies.
scoreboard players set Zombies temp.new_player_count 0
execute as @a[tag=dvz.zombie] run scoreboard players add Zombies temp.new_player_count 1

# Transfer the player count from the temporary scoreboard onto the real one.
scoreboard players operation Dwarves dvz.game.player_count = Dwarves temp.new_player_count
scoreboard players operation Zombies dvz.game.player_count = Zombies temp.new_player_count
