...
#> Note: ONLY runs when the player joins the server for the first time. dvz:tick/player_rejoin_server handles players rejoining the game.

# Set the player's gamemode to adventure mode. If the player is an admin, set their gamemode to creative mode.
gamemode adventure @s[predicate=!dvz:is_admin]
gamemode creative @s[predicate=dvz:is_admin]
...
scoreboard players enable @s[predicate=dvz:is_admin] admin_handbook
...
scoreboard players set @s dvz.misc.health 20
scoreboard players set @s dvz.misc.food 20
...
execute store result score @s dvz.game.reload_count run scoreboard players get &dvz dvz.game.reload_count

# Give the player the initilaized and lobby adventure mode tags. If the player is an admin, give them the admin tag as well.
tag @s add dvz.initialized
tag @s add dvz.adventure.lobby
tag @s[predicate=dvz:is_admin] add dvz.admin

# Give the player the default hero volunteer tag, so they can volunteer for hero later
tag @s add dvz.hero_default
...
execute as @s at @s run function dvz:tick/player_rejoin_server
