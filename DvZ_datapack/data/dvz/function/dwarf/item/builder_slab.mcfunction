...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:dwarf/item/builder_slab_loop
...
execute at @n[type=minecraft:marker,tag=temp.ray] positioned ^ ^ ^-0.001 run fill ^5 ^3 ^3 ^-5 ^-2 ^6 minecraft:end_stone_bricks replace #dvz:go_through
