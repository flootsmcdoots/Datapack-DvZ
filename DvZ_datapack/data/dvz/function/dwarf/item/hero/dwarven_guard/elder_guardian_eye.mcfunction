...
execute if entity @s[scores={dvz.dwarven_guard.elder_guardian_eye.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Elder Guardian Eye]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.dwarven_guard.elder_guardian_eye.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.dwarven_guard.elder_guardian_eye.cooldown.seconds=1..}] run return 0
...
scoreboard players remove @s dvz.dwarf.mana_buildup.mana 15

# Set the cooldown.
scoreboard players set @s dvz.dwarven_guard.elder_guardian_eye.cooldown.seconds 5
...
execute if entity @s[predicate=dvz:is_inside_water] as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:dwarf/item/hero/dwarven_guard/elder_guardian_eye_long_loop
execute unless entity @s[predicate=dvz:is_inside_water] as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:dwarf/item/hero/dwarven_guard/elder_guardian_eye_short_loop

# Deals damage to all players who have been hit by the ray. Damage is increased if the user is submerged in water.
execute if entity @s[predicate=dvz:is_inside_water] as @a[tag=temp.hit,tag=dvz.zombie] run damage @s 24 dvz:electric by @p[tag=temp.ray_origin]
execute unless entity @s[predicate=dvz:is_inside_water] as @a[tag=temp.hit,tag=dvz.zombie] run damage @s 16 dvz:electric by @p[tag=temp.ray_origin]
execute if entity @s[predicate=dvz:is_inside_water] as @a[tag=temp.hit,tag=dvz.dwarf] run damage @s 6 dvz:electric by @p[tag=temp.ray_origin]
execute unless entity @s[predicate=dvz:is_inside_water] as @a[tag=temp.hit,tag=dvz.dwarf] run damage @s 4 dvz:electric by @p[tag=temp.ray_origin]
