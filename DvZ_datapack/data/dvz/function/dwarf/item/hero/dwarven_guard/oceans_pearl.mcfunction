...
execute if entity @s[scores={dvz.dwarven_guard.oceans_pearl.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Ocean's Pearl]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.dwarven_guard.oceans_pearl.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.dwarven_guard.oceans_pearl.cooldown.seconds=1..}] run return 0
...
execute store result score @s temp.oceans_pearl_count if entity @e[type=minecraft:item_display,tag=dvz.item_display.oceans_pearl]
...
execute positioned ~ ~2.5 ~ if entity @e[type=minecraft:item_display,tag=dvz.item_display.oceans_pearl,distance=..12] run title @s actionbar [ \
  "", \
  {text:"[Ocean's Pearl]",bold:true,color:"red"}, \
  {text:" You are too close to another ocean's pearl!",color:"red"} \
]
execute positioned ~ ~2.5 ~ if entity @e[type=minecraft:item_display,tag=dvz.item_display.oceans_pearl,distance=..12] run return 0

# Display a fail message and return if there is not enough room to spawn an ocean's pearl.
execute unless block ~ ~2.5 ~ #dvz:go_through run title @s actionbar [ \
  "", \
  {text:"[Ocean's Pearl]",bold:true,color:"red"}, \
  {text:" There is not enough space!",color:"red"} \
]
execute unless block ~ ~2.5 ~ #dvz:go_through run return 0
...
scoreboard players remove @s dvz.dwarf.mana_buildup.mana 50

# Set the cooldown.
scoreboard players set @s dvz.dwarven_guard.oceans_pearl.cooldown.seconds 30
...
execute positioned ~ ~2.5 ~ align xyz positioned ~0.5 ~0.5 ~0.5 run summon minecraft:item_display ~ ~ ~ {teleport_duration:3,Tags:["dvz.item_display","dvz.item_display.oceans_pearl","dvz.kill_on_reload"],item:{id:"minecraft:heart_of_the_sea",count:1,components:{"minecraft:item_model":"dvz:oceans_pearl"}}}
...
execute positioned ~ ~2.5 ~ align xyz positioned ~0.5 ~0.11 ~0.5 run summon minecraft:slime ~ ~ ~ {Silent:1b,PersistenceRequired:1b,NoAI:1b,Health:40f,Tags:["dvz.oceans_pearl_hitbox","dvz.oceans_pearl_hitbox.resize","dvz.kill_on_reload"],active_effects:[{id:"minecraft:invisibility",amplifier:0,duration:-1,show_particles:0b},{id:"minecraft:regeneration",amplifier:2,duration:-1,show_particles:0b}],attributes:[{id:"minecraft:max_health",base:40},{id:"minecraft:scale",base:0.0}]}

# Resize the slime 5 ticks later.
schedule function dvz:dwarf/item/hero/dwarven_guard/oceans_pearl_resize_hitbox 5t append
