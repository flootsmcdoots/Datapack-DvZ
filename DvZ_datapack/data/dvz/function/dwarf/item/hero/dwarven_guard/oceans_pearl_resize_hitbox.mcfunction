...
execute as @e[type=minecraft:slime,tag=dvz.oceans_pearl_hitbox.resize] run attribute @s minecraft:scale base set 1.5

# Remove the resize tag from the pearl's hitbox.
tag @e[type=minecraft:slime,tag=dvz.oceans_pearl_hitbox.resize] remove dvz.oceans_pearl_hitbox.resize
