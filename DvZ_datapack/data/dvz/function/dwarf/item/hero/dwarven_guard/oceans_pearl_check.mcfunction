...
execute if entity @e[type=minecraft:slime,tag=dvz.oceans_pearl_hitbox,distance=..1] run return 0
...
execute as @a[tag=dvz.dwarf.class.hero.dwarven_guard] at @s run playsound minecraft:block.beacon.deactivate player @s ~ ~ ~ 1 0.5 1
playsound minecraft:block.beacon.deactivate player @a[tag=!dvz.dwarf.class.hero.dwarven_guard] ~ ~ ~ 1 0.5

# Display a broken ocean's pearl message to the dwarven guard.
title @a[tag=dvz.dwarf.class.hero.dwarven_guard] actionbar [ \
  "", \
...
scoreboard players set @a[tag=dvz.dwarf.class.hero.dwarven_guard] dvz.misc.custom_bar_hide.ticks 20
