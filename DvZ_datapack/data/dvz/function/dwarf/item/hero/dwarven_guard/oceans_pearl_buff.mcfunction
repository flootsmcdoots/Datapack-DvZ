...
tag @p[tag=dvz.dwarf,scores={dvz.dwarf.oceans_pearl_buff.cooldown.seconds=0},distance=..16] add temp.target
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:dwarf/item/hero/dwarven_guard/oceans_pearl_buff_loop
...
scoreboard players set @p[tag=temp.target] dvz.dwarf.oceans_pearl_buff.cooldown.seconds 8
