#> Description: Raycasting for dwarven guard's elder guardian eye for when they are outside of water.
...
execute unless entity @a[tag=temp.ray_origin,distance=..24] run return 0
...
execute as @s at @s run function dvz:dwarf/item/hero/dwarven_guard/elder_guardian_eye_short_loop