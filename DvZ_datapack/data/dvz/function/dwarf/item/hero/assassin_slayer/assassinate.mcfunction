...
execute if entity @s[scores={dvz.assassin_slayer.assassinate.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Assassinate]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.assassin_slayer.assassinate.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.assassin_slayer.assassinate.cooldown.seconds=1..}] run return 0
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:dwarf/item/hero/assassin_slayer/assassinate_loop
...
execute if entity @a[tag=temp.hit] run scoreboard players set @s dvz.assassin_slayer.assassinate.cooldown.seconds 8
