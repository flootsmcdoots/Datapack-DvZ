...
execute if entity @s[scores={dvz.assassin_slayer.mana_star.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Mana Star]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.assassin_slayer.mana_star.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.assassin_slayer.mana_star.cooldown.seconds=1..}] run return 0
...
# Set the cooldown.
scoreboard players set @s dvz.assassin_slayer.mana_star.cooldown.seconds 10
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:dwarf/item/hero/assassin_slayer/mana_star_loop

# Give mana regeneration (luck) to the hit dwarf.
effect give @p[tag=dvz.dwarf,tag=!dvz.dwarf.class.hero,tag=temp.hit] minecraft:luck 100 0
