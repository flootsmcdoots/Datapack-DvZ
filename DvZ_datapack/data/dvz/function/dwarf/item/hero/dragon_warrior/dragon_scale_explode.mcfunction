...
advancement revoke @s only dvz:misc/dragon_warrior_take_damage_from_own_firework_rocket_while_in_dragon_form

# Return if the player wasn't flying (can happen if they shoot themselves with a firework rocket shot from the crossbow).
execute as @s[predicate=!dvz:is_flying] run return 0
...
scoreboard players set @s dvz.dragon_warrior.dragon_scale.damage 10000
