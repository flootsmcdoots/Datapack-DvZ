...
execute if entity @s[scores={dvz.wither_warrior.phantom_slash.cooldown.seconds=1..}] run title @s actionbar [ \
  "", \
  {text:"[Phantom Slash]",bold:true,color:"red"}, \
  {text:" You have ",color:"red"}, \
  {"score":{"name":"@s","objective":"dvz.wither_warrior.phantom_slash.cooldown.seconds"},bold:true,color:"red"}, \
  {text:" seconds remaining!",color:"red"} \
]
execute if entity @s[scores={dvz.wither_warrior.phantom_slash.cooldown.seconds=1..}] run return 0
...
# Set the cooldown.
scoreboard players set @s dvz.wither_warrior.phantom_slash.cooldown.seconds 15
...
execute as @n[type=minecraft:marker,tag=temp.ray] at @s run function dvz:dwarf/item/hero/wither_warrior/phantom_slash_loop
...
execute at @n[type=minecraft:marker,tag=temp.ray] if block ~ ~-1.0 ~ #dvz:go_through if block ~ ~-1.6 ~ #dvz:go_through positioned ~ ~-1.6 ~ align xyz positioned ~0.5 ~ ~0.5 run tp @s ~ ~ ~
execute at @n[type=minecraft:marker,tag=temp.ray] if block ~ ~-1.0 ~ #dvz:go_through unless block ~ ~-1.6 ~ #dvz:go_through positioned ~ ~-1.0 ~ align xyz positioned ~0.5 ~ ~0.5 run tp @s ~ ~ ~
execute at @n[type=minecraft:marker,tag=temp.ray] unless block ~ ~-1.0 ~ #dvz:go_through positioned ~ ~ ~ align xyz positioned ~0.5 ~ ~0.5 run tp @s ~ ~ ~

# Deals damage to all players who have been hit by the ray.
execute as @a[tag=temp.hit,tag=dvz.zombie] run damage @s 15 minecraft:player_attack by @p[tag=temp.ray_caster]
execute as @a[tag=temp.hit,tag=dvz.dwarf] run damage @s 3.75 minecraft:player_attack by @p[tag=temp.ray_caster]

# Apply effects to all zombies hit.
effect give @a[tag=dvz.zombie,tag=temp.hit] minecraft:blindness 8 0
effect give @a[tag=dvz.zombie,tag=temp.hit] minecraft:slowness 8 0
effect give @a[tag=dvz.zombie,tag=temp.hit] minecraft:weakness 8 0
