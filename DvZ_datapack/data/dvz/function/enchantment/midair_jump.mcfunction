...
scoreboard players operation @s dvz.midair_jump.button_state.previous = @s dvz.midair_jump.button_state.current

# Get the new jump button state.
execute store success score @s dvz.midair_jump.button_state.current run execute if predicate dvz:input_jump_pressed

# Transfer the (now old) current is airborne scoreboard to the previous is airborne scoreboard.
scoreboard players operation @s dvz.midair_jump.is_airborne.previous = @s dvz.midair_jump.is_airborne.current

# Get the new is airborne score.
scoreboard players set @s dvz.midair_jump.is_airborne.current 1
scoreboard players set @s[predicate=dvz:is_on_ground] dvz.midair_jump.is_airborne.current 0

# Reset midair jumps and return if the player is grounded.
execute as @s[scores={dvz.midair_jump.is_airborne.current=0}] run return run scoreboard players set @s dvz.midair_jump.jumps 0

# Return if the player was previously grounded (the player likely jumped).
execute as @s[scores={dvz.midair_jump.is_airborne.previous=0}] run return 0

# Return if the player is swimming.
execute as @s[predicate=dvz:is_swimming] run return 0
...
execute as @s[scores={dvz.midair_jump.button_state.current=0}] run return 0
execute as @s[scores={dvz.midair_jump.button_state.previous=1}] run return 0
...
execute store result score @s temp.midair_jump.max_jumps run data get entity @s equipment.feet.components."minecraft:enchantments"."dvz:midair_jump"

# Subtract your current jumps from your max jumps to get how many jumps you have left.
scoreboard players operation @s temp.midair_jump.max_jumps -= @s dvz.midair_jump.jumps
...
scoreboard players add @s dvz.midair_jump.jumps 1
...
data modify storage dvz:vector y set from entity @s Pos[1]

# Run a macro that resets the player's y motion.
execute as @s run function dvz:macro/teleport_y with storage dvz:vector
...
attribute @s minecraft:explosion_knockback_resistance modifier add dvz:temp.remove.knockback_resistance -1 add_multiplied_total
...
execute as @s[scores={temp.jump_boost.amplifier=1..}] at @s run function dvz:enchantment/midair_jump_loop_1
execute as @s[scores={temp.jump_strength=1..}] at @s run function dvz:enchantment/midair_jump_loop_2

# Teleport the player back to where they've been.
tp @s ~ ~ ~

# Remove the attribute that disables knockback resistance.
attribute @s minecraft:explosion_knockback_resistance modifier remove dvz:temp.remove.knockback_resistance
